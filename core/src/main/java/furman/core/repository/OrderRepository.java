...
public interface OrderRepository extends JpaRepository<Order, Long>, QueryDslPredicateExecutor<Order>, JpaSpecificationExecutor {
    ...
    @Query(value = "select new furman.core.model.DateWithOrderCount(count(o), date(o.readyDate)) from Order o where o.status in :orderStatus group by date(o.createdDailySheet.date)")
